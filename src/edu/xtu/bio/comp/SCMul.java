...
import edu.xtu.bio.worker.WorkerParallelDisc;

public class SCMul  implements ScalableCompution{
	/**
	 * @author WuQi@XTU
	 * @time_created 2016年3月15日,上午10:01:39
	 * @version 1.0
	 */
	private static final Logger Log = Logger.getLogger(SCMul.class) ;
	...
	public SCMul(double[][] matrix, List<Species> list,Report r,boolean[] ktuple,ExecutorService se) {
		super();
		this.matrix = matrix;
		this.list = list;
		this.r = r;
	...
	private ProteinOptimalMultiDisc computeCV(Species e,boolean[] ktuple){
		...
		ProteinOptimalMultiDisc protein = new ProteinOptimalMultiDisc(e.getFilename() ,in,out,r.getMethod(),ktuple) ;
		FileUtil.mkdirs(out) ;
		if(!protein.start()){
			protein.freeAll();
			return null;
	...
	private void storeCV(ProteinOptimalMultiDisc protein,String path,boolean[] ktuple){
		String pathk = null ;
		if(ktuple[6]){
			pathk = path+"."+6;
	...
	private ProteinOptimalMultiDisc loadInnerProtein(Species s){
		
		ProteinOptimalMultiDisc p = new ProteinOptimalMultiDisc();
		p.setKtuple_out(ktuple);
		String path = getCVpath(s );
		boolean[] ktuple_redo = new boolean[7] ;
		int fail = 0 ;
		//开始加载
		for(int i=2;i<6;i++){
			if(this.ktuple[i]){
		...
		//开始加载
		if(this.ktuple[6]){
			p.setCv7(inner.loadGlobalKV(path+".6"));
			if(p.getCv7()==null){
				ktuple_redo[6]=true;
				fail++ ;
			}
		...
		if(fail!=0){
			//恢复加载失败部分
			ProteinOptimalMultiDisc rp = this.computeCV(s, ktuple_redo);
			if(rp==null){
				Log.error("loadInnerProtein,recover the part of load failure");
				return null;
			}
			if(ktuple_redo[6])p.setCv7(rp.getCv7());
			if(ktuple_redo[5])p.setCv6(rp.getCv6());
			if(ktuple_redo[4])p.setCv5(rp.getCv5());
			if(ktuple_redo[3])p.setCv4(rp.getCv4());
			if(ktuple_redo[2])p.setCv3(rp.getCv3());
		}
		return p;
	}
	
	private ProteinOptimalMultiDisc loadInnerProtein7(Species s){
	...
	public ProteinOptimalMultiDisc getProtein(int index){
	...
	private void clearBlock(List<Integer> currentList,List<ProteinOptimalMultiDisc> proteins){
	...
	private boolean submit(List<Future<ResultNew>> wait,List<ProteinOptimalMultiDisc> proteins,ProteinOptimalMultiDisc current_protein){
		if(wait.isEmpty())return true;
		WorkerParallelDisc work = null ;
		...
		for(int i=0;i<wait.size();i++){
			...
			if(!flag){
				if(current_protein==null){
					work = new WorkerParallelDisc(proteins.get(result.getRaw()),proteins.get(result.getColumn()), matrix, r, result.getK(), result.getRaw(), result.getColumn(), result.getSrc(), result.getDst());
				}else{
					work = new WorkerParallelDisc( proteins.get(result.getRaw()),current_protein, matrix, r, result.getK(), result.getRaw(), result.getColumn(), result.getSrc(), result.getDst());
				}
	...
	private boolean computeInnerBlock(List<Integer> currentList,List<ProteinOptimalMultiDisc> proteins){
				...
				for (int k = 2; k < this.ktuple.length; k++) {
					if (this.ktuple[k]) {
						...
						if (this.matrix[raw][column] < -1) {
							...
							work = new WorkerParallelDisc( proteins.get(raw),proteins.get(column) , matrix, r, k, raw, column, src, dst);
	...
	private boolean computeOuterBlock(int next,List<Integer> currentList,List<ProteinOptimalMultiDisc> proteins){
		if(next==list.size())return true ;
		WorkerParallelDisc work = null ;
		List<Future<ResultNew>> wait = new ArrayList<Future<ResultNew>>();
		ProteinOptimalMultiDisc current_protein = null ;
				...
				for (int k = 2; k < this.ktuple.length; k++) {
					if (this.ktuple[k]) {
						if (this.matrix[raw][column] < -1) {
							...
							work = new WorkerParallelDisc( proteins.get(raw),current_protein,matrix, r, k, raw, column, src, dst);
							wait.add( se.submit(work) ) ;
						}
						break;
					}
	...
	private long getFullVectorMemory(){
		 ...
		 * 
		 **/
		for (int i = 0; i < ktuple.length-1; i++) {
			if (ktuple_in[i])
				total += (long)ProteinAbstract.BASES[i+1] * 4L;
		}
		if (ktuple_in[ktuple.length-1]){
			total += (long)ProteinAbstract.BASES[ktuple.length-2] * 4L * (long)InnerDBBuilder.pool_max_size;
		}
		/**=====
	...
	private boolean initListSingle(){
		...
		ProteinOptimalMultiDisc p = null ;
		for(Species s:list){
	...
	private boolean submitLoader(List<Future<ResultLoader>> wait,List<ProteinOptimalMultiDisc> proteins,List<Integer> currentList){
		try {
			ResultLoader result = null;
			for (Future<ResultLoader> r : wait) {
				result = r.get();
				if (result.isStatus()) {
					proteins.set(result.getIndex(), (ProteinOptimalMultiDisc)result.getO());
	...
	public boolean doloop() {
		...
		// null表示 未加载或已卸载
		List<ProteinOptimalMultiDisc> proteins = new ArrayList<ProteinOptimalMultiDisc>(list.size());
		// 初始化
		for (int i = 0; i < list.size(); i++) {
			proteins.add(null);
		}
		return this.ktuple[6]?this.doloopSingle(currentList, proteins):this.doloopMulti(currentList, proteins);
	}
	
	public boolean doloopSingle(List<Integer> currentList,List<ProteinOptimalMultiDisc> proteins){
		...
		while(next<list.size()-1){
			...
			ProteinOptimalMultiDisc protein =  getProtein(next) ;//获取头指标的加载值
			if(protein==null){
	...
	public boolean doloopMulti(List<Integer> currentList,List<ProteinOptimalMultiDisc> proteins){
		//由next指标 的数据使用量 来确定 当前最大加载数
		long max = 0 ;
		long max_parallel = 0 ;
		long max_next = 0 ;
		...
		while(next<list.size()-1){
			...
			if(ret==-1){
				Log.error("loader proteins fail");
				return false;
			}else if(ret==0){
				break;
			}
	...
	public int loaderSingleThread(List<ProteinOptimalMultiDisc> proteins,List<Integer> currentList){
		ProteinOptimalMultiDisc protein = null ;
	...
	public int loaderSingleThread(long max,List<ProteinOptimalMultiDisc> proteins,List<Integer> currentList){
		ProteinOptimalMultiDisc protein = null ;
		for(int i=0;i<max;i++){
			next++;
			next = nextLoaded(next,currentList);
	...
	public int loaderMultiThread(long max,long max_parallel,List<ProteinOptimalMultiDisc> proteins,
